...
import { getDateBucketLabel, getDateBucketBoundaries, bucketSessionsByDate } from '@/lib/date-buckets';
...
describe('getDateBucketBoundaries', () => {
  ...
  it('returns boundaries where each is before the next', () => {
    const { startOfToday, startOfYesterday, startOfWeek, startOfLastWeek } = getDateBucketBoundaries(NOW_MS);
